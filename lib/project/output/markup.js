...
function frames(data)
{
	...
	data.project.media.forEach( function(file, i)
	{
		...
		//str += '>data:image/'+file.type+';base64,'+ file.contents.toString("base64") +'</image></g>';
	});
...
function masks(data)
{
	...
	data.project.media.forEach( function(file, i)
	{
		if (file.matte instanceof Buffer)
		{
			...
			// TODO :: does this work in all browsers?
			//str += '<mask id="'+frameLabel+i+maskLabel+'">';
			//str += '<image width="'+file.width+'" height="'+file.height+'">data:image/'+file.type+';base64,'+ file.matte.toString("base64") +'</image>';
			//str += '</mask>';
		}
	});
