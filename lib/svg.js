...
function getSVG(files, callback)
{
	...
	svg = getSVG_smil(files);
	svg = getSVG_css(svg);
	
	//getSVG_optimized( new Buffer(svg), function(svg)
	//{
		callback(null, svg);
	//});
}
...
function getSVG_css(svg)
{
	return smil2css.convert(svg);
}



function getSVG_optimized(svg, callback)
{
	imageminInstance.run( svg, function(error, data)
	{
